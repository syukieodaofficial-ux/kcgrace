(function(){
  ...
  const faqs = [
    { q: ['volunteer','volunte','volunteer program'], a: 'To volunteer, please join our Monthly Youth Forum or contact us via email: bandoykc6@gmail.com' },
    { q: ['workshop','training','leadership workshop'], a: 'Workshops are announced in Updates and on our Facebook page. Next workshop dates are posted there.' },
    { q: ['contact','phone','email'], a: 'You can reach KC at 0951 818 2771 or bandoykc6@gmail.com' },
    { q: ['education','school','bsed'], a: 'KC is currently pursuing BSEd English (Ongoing).' },
    { q: ['events','community','clean-up','clean up'], a: 'Check the Updates section for recent and ongoing community activities.' }
  ];
  ...
  // Local cached Facebook feed (optional). Fetches /fb-feed.json if present.
  let fbFeed = null;
  fetch('fb-feed.json').then(r => {
  ...
  function findAnswer(text){
    ...
    if (t.includes('update') || t.includes('latest') || t.includes("what's new") || t.includes('recent') || t.includes('balita') || t.includes('ganap') || t.includes('anong bago') || t.includes('ano ang bago')){
      if (fbFeed && Array.isArray(fbFeed.posts) && fbFeed.posts.length){
        const latest = fbFeed.posts.slice(0,3).map(p => {
          const date = new Date(p.created_time);
          return `📌 ${date.toLocaleDateString()}:\n${p.message}\n🔗 Link: ${p.permalink_url || fbFeed.url}`;
        }).join('\n');
        return 'Narito ang mga huling balita:\n\n' + latest + '\n\nPara sa iba pang detalye, bisitahin ang profile ni KC dito: https://www.facebook.com/profile.php?id=61581987389724';
      } else {
        return 'Sa ngayon ay wala pa akong makitang bagong updates. Maaari mong tignan ang profile ni KC para sa live feed dito: https://www.facebook.com/profile.php?id=61581987389724. Maraming salamat!';
      }
    }
